...
class Scraper
{
    ...
    {
        return new static($options, $timeout);
    }
    ...
    public function refreshGuestToken()
    {
        $query = $this->query ?? 'twitter';
    ...
    protected function query(string $query, ?DateTime $start = null, ?DateTime $end = null)
    {
        ...
        while ($retries < 5) {
            try {
                ...
                sleep($retries);
                $retries++;
                // if ($e instanceof \Jerrkill\Twgetter\Exceptions\TokenExpiryException) {
                //     $message = 'TokenExpiryException : '.$e->getMessage();
                // }
            }
        }
        // if ($lastDate !== null) {
    ...
    protected function queryInterval(string $query)
    {
        ...
        do {
            try {
                $response = $this->client->request('GET', "https://api.twitter.com/2/search/adaptive.json?include_profile_interstitial_type=1&include_quote_count=1&include_blocking=1&include_blocked_by=1&include_followed_by=1&include_want_retweets=false&include_mute_edge=1&include_can_dm=1&include_can_media_tag=1&skip_status=1&cards_platform=Web-12&include_cards=1&include_composer_source=true&include_ext_alt_text=true&include_reply_count=1&tweet_mode=extended&include_entities=true&include_user_entities=true&include_ext_media_color=true&include_ext_media_availability=true&send_error_codes=true&q={$query}&tweet_search_mode=live&count={$this->chunkSize}&query_source=typed_query&cursor={$cursor}&pc=1&spelling_corrections=1&ext=mediaStats%2ChighlightedLabel%2CcameraMoment");
            ...
            if ($this->fetchedTweets >= $this->total) {
                $shouldScroll = false;
            }
            $cursor = $newCursor;
